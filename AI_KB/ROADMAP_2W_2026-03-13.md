# Diagen 近期开发计划（1-2 周，基于 `.processon` 与 draw.io 对照）

日期：2026-03-13  
计划周期：10 个工作日（可在 1-2 周内按人力压缩或拉伸）  
计划目标：在不破坏现有分层（`core/renderer/primitives`）的前提下，优先补齐编辑器生产力缺口。

---

## 0. 分析基线（本计划依据）

1. `.processon` 源码锚点：
- 吸附线：`designer.core.js:snapLine(5769)`、`snapResizeLine(5978)`
- 画布扩容：`designer.core.js:changeCanvas(6633)`
- 剪贴板：`designer.core.js:clipboard(6663+)`
- 控件反馈：`designer.core.js:drawControls(8861)`、`showLinkerControls(10575)`、`showAnchors(10689)`
- 历史系统：`designer.core.js:undoStack/redoStack(13144+)` + `designer.events.js:undoStackChanged/clipboardChanged(212+)`

2. draw.io / mxGraph 参考轴：
- `View/Model` 分离
- `Handler` 原语化交互体系
- `Overlay` 反馈层
- `Undo` 事务边界
- 路由与可读性（含 line jumps）

3. Diagen 当前对照入口：
- 坐标归一化：`packages/renderer/src/primitives/createCoordinateService.ts`
- 交互状态机：`packages/renderer/src/primitives/createInteractionMachine.ts`
- 历史事务：`packages/core/src/designer/managers/history.ts`
- 视图管理：`packages/core/src/designer/managers/view.ts`

---

## 1. 目标与成功标准

### 1.1 目标（P0/P1）
1. P0：补齐 shape move/resize 的吸附线（Guide）闭环。
2. P0：落地 `clipboard manager`（copy/cut/paste/duplicate）并接入历史事务。
3. P1：将现有 router 能力接入主链路，打通 `lineJumps` 渲染开关。
4. P1：为核心交互补齐回归测试与 playground 验证脚本。

### 1.2 成功标准（验收）
1. 拖拽/缩放时，吸附线与吸附行为稳定可见，误吸附可控（阈值可配置）。
2. 复制/剪切/粘贴支持多选、group 保真、ID 重映射，撤销重做行为正确。
3. 连线路由切换后可稳定渲染，`lineJumps` 开关生效且无明显性能回退。
4. 新增测试覆盖关键路径，playground 可复现并验证以上能力。

---

## 2. 范围定义

### 2.1 本周期内（In Scope）
1. Guide 计算引擎（core）+ Guide overlay（renderer）。
2. Clipboard 语义与命令入口（core 优先，UI 仅薄层绑定）。
3. Router 主链路接入与 `lineJumps` 的可配置渲染。
4. 单元测试与最小集成回归（交互链路）。

### 2.2 本周期外（Out of Scope）
1. 评论系统完整产品化（仅保留接口位）。
2. 全量导出体系（PNG/PDF/SVG 完整参数面板）。
3. 实时协作协议与远端冲突合并。

---

## 3. 详细执行计划（D1-D10）

### Week 1

### D1：技术方案冻结与任务拆解
1. 完成 Guide/Clipboard/Router 的接口草案与数据流图。
2. 明确事务边界：一个用户动作对应一个 history entry。
3. 输出：`AI_KB` 设计补充条目 + 开发任务清单（issue/todo）。

验收：接口签名、事件名、状态流得到统一；无跨层职责冲突。

### D2：Guide 计算层（move）
1. 在 `core` 新增 guide candidate 收集与最近吸附点求解。
2. 提供移动场景 `snap(delta)` 能力（中心线/边线）。
3. 输出：可单测的纯函数与阈值配置项。

验收：给定输入集合可稳定输出吸附偏移与 guide line 数据。

### D3：Guide 计算层（resize）
1. 补齐 resize 方向约束（N/S/E/W/NE/NW/SE/SW）。
2. 统一 move/resize 结果结构，避免 renderer 分支爆炸。
3. 输出：resize 专项单测（边界尺寸、最小尺寸、反向拖拽）。

验收：resize 时吸附行为与方向一致，不产生非法尺寸。

### D4：Guide Overlay 接入
1. 在 `renderer` 的 `overlay-layer` 渲染 guide lines。
2. 接入 `createShapeDrag` 与 `createResize` 会话反馈。
3. 输出：playground 可视化验证（拖拽/缩放时显示与消失时机正确）。

验收：无残影、无闪烁、无坐标漂移；缩放后位置准确。

### D5：Week1 集成与回归
1. 清理 API 命名与事件类型，补文档。
2. 执行核心回归测试（选择、拖拽、缩放、撤销重做）。
3. 输出：Week1 集成报告（问题清单 + 修复优先级）。

验收：P0 功能基础闭环可用，已知问题可追踪。

### Week 2

### D6：Clipboard Manager 设计与实现（core）
1. 新增 clipboard 数据结构与序列化协议。
2. 实现 `copy/cut/paste/duplicate`，支持 ID 重映射。
3. 输出：group/linker 引用关系保持策略（含降级规则）。

验收：多选复制后粘贴结果结构完整、引用正确、无 ID 冲突。

### D7：Clipboard 与历史/交互接入
1. 将 clipboard 动作纳入 transaction scope。
2. 绑定快捷键/命令入口（renderer 或 playground 层）。
3. 输出：撤销重做覆盖 copy/cut/paste/duplicate。

验收：每次粘贴为单独可撤销单元；cut 可完整恢复。

### D8：Router 主链路接入 + lineJumps
1. 将 `packages/core/src/utils/router/*` 接入 linker layout 主流程。
2. 打通页面 `lineJumps` 配置到渲染层。
3. 输出：路由策略切换与跳线显示 demo。

验收：密集连线场景可读性提升；开启/关闭行为一致。

### D9：性能与稳定性专项
1. 构造中等复杂场景（节点/连线规模）进行交互回归。
2. 关注拖拽帧率、overlay 更新频率、history 体积增长。
3. 输出：性能基线与优化清单（必要时分批修复）。

验收：关键交互无明显卡顿，无持续性内存增长风险。

### D10：封板与知识库同步
1. 合并本周期文档：架构、集成点、对照结论。
2. 形成版本说明（新增能力、限制、已知问题）。
3. 输出：下一周期滚动 backlog（按优先级排序）。

验收：功能、文档、测试三者一致，可进入下一迭代。

---

## 4. 交付物清单（本周期）

1. Guide：计算模块 + overlay 渲染 + 交互接线 + 测试。
2. Clipboard：manager + 命令入口 + 历史事务接入 + 测试。
3. Router/LineJumps：主链路接入 + 配置开关 + 回归用例。
4. 文档：`AI_KB` 对应章节更新（架构、集成点、路线图）。

---

## 5. 风险、依赖与缓解

1. 风险：Guide 逻辑侵入交互 handler，导致职责混乱。  
缓解：Guide 只在 core 计算，renderer 仅消费结果并绘制。

2. 风险：Clipboard 与 group/linker 关系重建出现脏引用。  
缓解：先做纯模型层单测，再接 UI 命令入口。

3. 风险：Router 接入后性能回退。  
缓解：保留基础策略兜底；新增配置可降级。

4. 依赖：统一 history 事件类型与事务提交时机。  
缓解：D1 先冻结事件契约，D5/D9 做集中回归。

---

## 6. 测试策略（最小必做）

1. 单元测试：
- Guide：候选线生成、吸附偏移计算、阈值边界。
- Clipboard：ID 重映射、group 保真、linker 端点降级。
- Router：策略切换后路径合法性、lineJumps 交叉点处理。

2. 集成回归：
- 拖拽/缩放/连线/粘贴/撤销重做串行场景。
- 缩放比例变化下坐标一致性（依赖 `createCoordinateService`）。

3. 手工验证（playground）：
- 中高密度图场景操作 10-15 分钟，观察稳定性与性能。

---

## 7. 两周后计划（简版，滚动动态调整）

1. P2：评论系统最小闭环（锚点、面板、持久化接口），按产品反馈拆分。
2. P2：导出能力分阶段补齐（先 SVG/PNG，再 PDF 与高级选项）。
3. P3：协作能力预研（操作日志协议、冲突策略、远端回放）。
4. 每周固定一次滚动评审：只保留下一周的细化计划，后续保持轻量 backlog。

---

## 8. 计划维护规则

1. 本文是近期执行基线，按周更新，不做长期瀑布式承诺。
2. 若 P0 未完成，不并行开启新 P2 功能开发。
3. 每次调整必须同步更新 `AI_KB/README.md` 的“当前版本要点”与本文件日期。
