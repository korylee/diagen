...
import { createSceneMouseDown } from './createSceneMouseDown'

const designerCtx = vi.hoisted(() => ({
  current: null as ReturnType<typeof createDesigner> | null,
}))

vi.mock('../../context/DesignerProvider', () => ({
  useDesigner: () => {
...
function withSceneMouseDown(
  run: (ctx: {
    ...
    onSceneMouseDown: (event: MouseEvent) => boolean
  }) => void,
) {
  createRoot(dispose => {
    ...
    const onSceneMouseDown = createSceneMouseDown({
...
describe('createSceneMouseDown', () => {
  ...
  it('点击空白区域时应清空选择并进入框选', () => {
    withSceneMouseDown(({ designer, pointer, onSceneMouseDown }) => {
      ...
      expect(onSceneMouseDown(event)).toBe(true)
  ...
  it('create-shape 非连续模式下应创建图形并回到 idle', () => {
    withSceneMouseDown(({ designer, onSceneMouseDown }) => {
