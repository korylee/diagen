...
import { resolveAnchors, getAnchorAngle } from '@diagen/core'
...
export function getShapeAnchorPosition(shape: ShapeElement, anchorIndex: number): Point | null {
  ...
  if (!anchors[anchorIndex]) return null
  return { x: shape.props.x + anchors[anchorIndex].x, y: shape.props.y + anchors[anchorIndex].y }
}
