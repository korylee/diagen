...
describe('transform', () => {
  describe('screenToCanvas', () => {
    it('应正确将屏幕点转换为画布点（含 originOffset）', () => {
      const transform: Transform = { x: 120, y: 80, zoom: 2 }
      const originOffset: Point = { x: 30, y: 10 }
      const point: Point = { x: 250, y: 170 }

      const result = screenToCanvas(point, transform, originOffset)

      expect(result).toEqual({ x: 50, y: 40 })
    })

    it('应正确将屏幕矩形转换为画布矩形（宽高应除以 zoom）', () => {
      const transform: Transform = { x: 40, y: 20, zoom: 2.5 }
      const screenBounds: Bounds = { x: 165, y: 95, w: 250, h: 125 }

      const result = screenToCanvas(screenBounds, transform)
  ...
  describe('canvasToScreen + screenToCanvas', () => {
    it('点坐标应满足可逆性', () => {
      const transform: Transform = { x: -32, y: 145, zoom: 1.75 }
      const originOffset: Point = { x: 24, y: -16 }
      const canvasPoint: Point = { x: 81.2, y: -43.6 }

      const screenPoint = canvasToScreen(canvasPoint, transform, originOffset)
      const restoredPoint = screenToCanvas(screenPoint, transform, originOffset)
    ...
    it('矩形坐标应满足可逆性', () => {
      const transform: Transform = { x: 220, y: -90, zoom: 0.75 }
      const originOffset: Point = { x: -12, y: 36 }
      const canvasBounds: Bounds = { x: -100, y: 50, w: 240, h: 120 }

      const screenBounds = canvasToScreen(canvasBounds, transform, originOffset)
      const restoredBounds = screenToCanvas(screenBounds, transform, originOffset)
